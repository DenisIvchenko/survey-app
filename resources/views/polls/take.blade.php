<x-guest-layout>
    {{--  СТИЛИ ДЛЯ ПОЛНОЭКРАННОГО РЕЖИМА + АНИМАЦИИ --}}
    <style>
    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-[#0b0b0f] dark:to-[#121218] py-6 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-7xl mx-auto">
            ...
            {{--  Форма прохождения — x-data ОБЪЯВЛЕН ЗДЕСЬ, прогресс-бар ВНУТРИ --}}
            <form action="{{ route('polls.submit', $poll) }}" method="POST" 
                  x-data="{ 
                      currentStep: 0, 
                      answers: {},
                ...
                {{--  ЕДИНЫЙ прогресс-бар (внутри x-data, видит currentStep) --}}
                <div class="bg-white/80 dark:bg-[#1a1a20]/80 backdrop-blur-xl rounded-3xl shadow-xl border border-gray-200/50 dark:border-white/10 p-6 sm:p-8 mb-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                        <div>
                            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">{{ $poll->title }}</h1>
                            @if($poll->description)<p class="text-gray-600 dark:text-gray-400 text-sm mt-1">{{ $poll->description }}</p>@endif
                        </div>
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-white/5 px-3 py-1.5 rounded-full">
                            Вопрос <span x-text="currentStep + 1"></span> из {{ $poll->questions->count() }}
                        </span>
                    </div>
                    ...
                    <div class="w-full bg-gray-200 dark:bg-white/10 rounded-full h-2 overflow-hidden">
                        <div class="bg-gradient-to-r from-blue-500 to-blue-600 h-2 rounded-full transition-all duration-500 ease-out"
                             x-bind:style="`width: ${((currentStep + 1) / {{ $poll->questions->count() }}) * 100}%`"></div>
                    </div>
                </div>
                    ...
                    <div x-show="currentStep === {{ $index }}" 
                        ...
                        <div class="mb-6">
                            <div class="flex items-start gap-4">
                                <span class="flex-shrink-0 w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold text-lg">{{ $index + 1 }}</span>
                                <div class="flex-1 min-w-0">
                                    <h2 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white mb-1">
                                        {{ $question->text }}@if($question->is_required)<span class="text-red-500 ml-1">*</span>@endif
                                    </h2>
                                    @if($question->description)<p class="text-sm text-gray-500 dark:text-gray-400">{{ $question->description }}</p>@endif
                                </div>
                            </div>
                        </div>

                        {{--  Тип: Текст / Число / Контакт --}}
                        @if(in_array($question->type, ['text', 'number', 'contact_email', 'contact_phone']))
                            <div class="max-w-2xl">
                                <input type="{{ $question->type === 'contact_email' ? 'email' : ($question->type === 'number' ? 'number' : 'text') }}"
                                       name="answers[{{ $question->id }}]"
                                       @required($question->is_required)
                                       placeholder="{{ is_array($question->options) ? ($question->options['placeholder'] ?? 'Ваш ответ') : 'Ваш ответ' }}"
                            ...
                            <div class="max-w-2xl space-y-3">
                                @foreach((is_array($question->options) ? $question->options : []) as $option)
                                    <label class="flex items-center gap-4 p-4 rounded-2xl border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 hover:border-blue-300 dark:hover:border-blue-500/50 cursor-pointer transition-all group">
                                        <input type="radio" name="answers[{{ $question->id }}]" value="{{ $loop->index }}" @required($question->is_required) class="w-5 h-5 text-blue-600 border-gray-300 focus:ring-blue-500">
                                        <span class="text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">{{ $option }}</span>
                                    </label>
                                @endforeach
                            </div>

                        {{--  Тип: Выбор нескольких (checkbox) --}}
                        @elseif($question->type === 'checkbox')
                            <div class="max-w-2xl space-y-3">
                                @foreach((is_array($question->options) ? $question->options : []) as $option)
                                    <label class="flex items-center gap-4 p-4 rounded-2xl border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 hover:border-blue-300 dark:hover:border-blue-500/50 cursor-pointer transition-all group">
                                        <input type="checkbox" name="answers[{{ $question->id }}][]" value="{{ $loop->index }}" class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                        <span class="text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">{{ $option }}</span>
                                    </label>
                                @endforeach
                            </div>

                        {{--  Тип: Медиа (изображения для выбора) --}}
                        @elseif($question->type === 'media')
                            ...
                            @if(count($images) > 0)
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
                                    @foreach($images as $idx => $option)
                                        <label class="media-card relative cursor-pointer group block">
                                            <input type="{{ $multiple ? 'checkbox' : 'radio' }}" name="answers[{{ $question->id }}]{{ $multiple ? '[]' : '' }}" value="{{ $idx }}" @required($question->is_required) class="absolute inset-0 opacity-0 cursor-pointer z-20 peer" onchange="this.closest('label').classList.toggle('selected', this.checked)">
                                            <div class="aspect-square rounded-2xl overflow-hidden border-2 peer-checked:border-blue-500 peer-checked:ring-2 peer-checked:ring-blue-500/30 border-gray-200 dark:border-white/10 transition-all duration-200 group-hover:border-blue-400 bg-gray-100 dark:bg-white/5">
                                                <img src="{{ asset('storage/' . ($option['path'] ?? '')) }}" class="w-full h-full object-cover" onerror="this.src='https://via.placeholder.com/200x200/9ca3af/ffffff?text=No+Image'">
                                            ...
                                            <span class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent text-white text-[10px] sm:text-xs py-2 px-3 truncate">{{ $option['name'] ?? 'Изображение #' . ($idx + 1) }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            @else
                            ...
                            <div class="max-w-2xl">
                                <label class="block border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-2xl p-6 text-center hover:border-blue-400 dark:hover:border-blue-500 transition-colors cursor-pointer bg-gray-50/50 dark:bg-white/5">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <input type="file" name="answers[{{ $question->id }}][]" multiple @required($question->is_required) accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,.zip,.rar" class="hidden">
                        ...
                        {{--  Тип: Загрузка изображений --}}
                        @elseif($question->type === 'image_upload')
                            <div class="max-w-2xl">
                                <label class="block border-2 border-dashed border-blue-300 dark:border-blue-500/40 bg-blue-50/50 dark:bg-blue-500/10 rounded-2xl p-6 text-center hover:border-blue-400 transition-colors cursor-pointer">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <input type="file" name="answers[{{ $question->id }}][]" multiple @required($question->is_required) accept="image/*" class="hidden">
                            ...
                            <div class="flex gap-2 justify-center">
                                @for($i = 1; $i <= ($question->max ?? 5); $i++)
                                    <label class="cursor-pointer group">
                                        <input type="radio" name="answers[{{ $question->id }}]" value="{{ $i }}" @required($question->is_required) class="peer sr-only">
                        ...
                        {{--  Тип: Смайл-рейтинг --}}
                        @elseif($question->type === 'smiley_rating')
                            <div class="flex gap-3 sm:gap-4 justify-center">
                                @foreach(['😞','😟','😐','🙂','😊'] as $idx => $emoji)
                                    <label class="cursor-pointer group">
                                        <input type="radio" name="answers[{{ $question->id }}]" value="{{ $idx + 1 }}" @required($question->is_required) class="peer sr-only">
                                        <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center text-2xl sm:text-3xl peer-checked:bg-blue-100 dark:peer-checked:bg-blue-500/20 peer-checked:scale-110 bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 transition-all duration-200" x-text="'{{ $emoji }}'"></div>
                                    </label>
                                @endforeach
                            </div>
                        ...
                        @elseif($question->type === 'yesno')
                            <div class="flex gap-4 justify-center max-w-xs mx-auto">
                                <label class="flex-1">
                                    <input type="radio" name="answers[{{ $question->id }}]" value="1" @required($question->is_required) class="peer sr-only">
                                ...
                                <label class="flex-1">
                                    <input type="radio" name="answers[{{ $question->id }}]" value="0" @required($question->is_required) class="peer sr-only">
                        ...
                        @elseif($question->type === 'dropdown')
                            <div class="max-w-2xl">
                                <select name="answers[{{ $question->id }}]" @required($question->is_required) class="w-full px-5 py-4 rounded-2xl border border-gray-300 dark:border-white/10 bg-gray-50 dark:bg-[#121216] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    <option value="" disabled selected>Выберите вариант...</option>
                                    @foreach((is_array($question->options) ? $question->options : []) as $option)<option value="{{ $loop->index }}">{{ $option }}</option>@endforeach
                                </select>
                            </div>
                        ...
                        @elseif($question->type === 'slider')
                            <div class="max-w-2xl space-y-4">
                                <div class="flex items-center gap-4">
                                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ $question->min ?? 0 }}</span>
                                    <input type="range" name="answers[{{ $question->id }}]" min="{{ $question->min ?? 0 }}" max="{{ $question->max ?? 100 }}" @required($question->is_required) class="flex-1 h-2 bg-gray-200 dark:bg-white/10 rounded-lg appearance-none cursor-pointer accent-blue-600" oninput="this.nextElementSibling.value = this.value">
                                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ $question->max ?? 100 }}</span>
                                </div>
                                <output class="block text-center text-lg font-semibold text-blue-600 dark:text-blue-400">{{ $question->min ?? 0 }}</output>
                            </div>
                        ...
                        @elseif($question->type === 'scale')
                            <div class="max-w-2xl space-y-4">
                                <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400">
                                    <span>{{ $question->left_label ?? 'Не согласен' }}</span>
                                    <span>{{ $question->right_label ?? 'Согласен' }}</span>
                                </div>
                        ...
                        {{--  Тип: Распределительная шкала --}}
                        @elseif($question->type === 'distribution_scale')
                            <div class="max-w-2xl space-y-4">
                                <div class="flex items-center gap-4">
                                    <span class="text-xs text-gray-400">{{ $question->min ?? 0 }}%</span>
                                    <input type="range" name="answers[{{ $question->id }}]" min="{{ $question->min ?? 0 }}" max="{{ $question->max ?? 100 }}" @required($question->is_required) class="flex-1 h-2 bg-gray-200 dark:bg-white/10 rounded-lg appearance-none cursor-pointer accent-blue-600">
                                    <span class="text-xs text-gray-400">{{ $question->max ?? 100 }}%</span>
                                </div>
                            </div>

                        {{--  Тип: Семантический дифференциал --}}
                        @elseif($question->type === 'semantic_differential')
                            <div class="max-w-2xl space-y-4">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-300">{{ $question->left ?? 'Негативно' }}</span>
                                    <span class="text-gray-600 dark:text-gray-300">{{ $question->right ?? 'Позитивно' }}</span>
                                </div>
                        ...
                        {{--  Тип: Ранжирование / Матрица (базовая визуализация) --}}
                        @elseif(in_array($question->type, ['ranking', 'matrix']))
                            <div class="max-w-2xl text-center py-8 bg-gray-50 dark:bg-white/5 rounded-2xl border-2 border-dashed border-gray-200 dark:border-white/10">
                                <p class="text-sm text-gray-500 dark:text-gray-400">Вопрос типа «{{ $question->type }}» доступен для прохождения в упрощённом режиме</p>
                                <input type="text" name="answers[{{ $question->id }}]" placeholder="Ваш комментарий (опционально)" class="mt-4 w-full max-w-md mx-auto px-4 py-3 rounded-xl border border-gray-300 dark:border-white/10 bg-gray-50 dark:bg-[#121216] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                        @endif
                        ...
                        <div class="flex justify-between items-center mt-8 pt-6 border-t border-gray-200 dark:border-white/10">
                            ...
                            @else
                                <div></div>
                            @endif
                            @if($index < $poll->questions->count() - 1)
            ...
            <div class="mt-6 flex justify-center pb-8">
                <div class="flex items-center gap-2">
                    @for($i = 0; $i < $poll->questions->count(); $i++)
                        <button type="button" @click="currentStep = {{ $i }}" class="w-2.5 h-2.5 rounded-full transition-all" :class="currentStep === {{ $i }} ? 'bg-blue-600 w-6' : 'bg-gray-300 dark:bg-white/20 hover:bg-gray-400'"></button>
                    @endfor
                </div>
            </div>
        </div>
